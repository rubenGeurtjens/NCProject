"""
Trains U-net one with collorJitter
"""
...
def save_img(loader, model, folder="saved_images/", device="cuda"):
...
def eval_model(loader, model, writer):
    ...
    with torch.no_grad():
        for x, y in loader:
            ...
            num_correct += (outcome == y).sum()
            num_pixels += torch.numel(outcome)
            dice_score += (2 * (outcome * y).sum()) / (
                (outcome + y).sum() + 1e-8
            )
...
if __name__ == "__main__":
    ...
    #model.load_state_dict(torch.load('/home/ruben/Documents/school/Master 1/natural computing/NaturalComputing_assignments/results_project/results/models/u_net_one_color'))
